      ...
      <div class="card mb-3 acontecimiento-card" data-acontecimiento-id="${event.acontecimiento_id}">
        <div class="card-body">
          ...
          <div class="d-flex align-items-start mb-3">
            <img class="me-2 avatar-sm rounded-circle" src="${baseUrl}public/assets/images/users/avatar-1.png" alt="Generic placeholder image">
            <div class="w-100">
...
window.openImageCarousel = function (eventIndex, photoIndex) {
  const modal = new bootstrap.Modal(
    document.getElementById("imageCarouselModal"),
  );
...
window.cerrarAcontecimiento = async function (acontecimiento_id) {
  ...
  const card = document.querySelector(
    `.acontecimiento-card[data-acontecimiento-id="${acontecimiento_id}"]`,
  );
  if (!card) {
  ...
  if (button) {
    ...
    button.innerHTML =
      '<span class="spinner-border spinner-border-sm me-1" role="status" aria-hidden="true"></span>Cerrando...';
  }
  ...
  try {
    ...
    const response = await fetch(
      `${baseUrl}api/acontecimientos/${acontecimiento_id}/estado`,
      {
        method: "POST",
        body: formData,
      },
