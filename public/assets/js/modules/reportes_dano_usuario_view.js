...
function initFiltersAndButtons() {
  ...
  $('#btnResetFilters').on('click', () => {
    ...
    // Limpiar el primer filtro y disparar 'change' para recargar la cascada
    // y disparar el refresco al final
  ...
  $('#filtroAprisco').on('change', async function () {
    ...
    // [CAMBIO] Quitar refresco de aquí para evitar múltiples llamadas
  ...
  $('#filtroRecinto, #filtroEstado, #filtroCriticidad').on('change', () => {
    // Evitar que el 'change' de los padres (Finca, Aprisco, Area) refresque
...
async function handleRowAction(e) {
  ...
  if ($btn.hasClass('btn-eliminar')) {
    const ok = await Swal.fire({
      title: '¿Estás seguro?',
      text: 'El reporte será eliminado.',
      icon: 'warning',
...
/* =========================
   Renderizado y Reseteo (Sin cambios)
========================= */
...
function resetReporteForm() {
  ...
  if (form) {
    form.reset()
    $('#finca_id, #aprisco_id, #area_id, #recinto_id, #criticidad')
      .val(null)
      .trigger('change')
